(ns kiyo.app
  "etzhayyim-wasm-kiyo-k1y04rc4 appview — reagent + re-frame, view built
  ...
  The Svelte page never called the dispatcher or any kotoba/ reference
  slice — it was pure static markup describing the surface's own metadata,
  and this port adds no research-archive functionality that was not
  already there.

  The Svelte scaffold's `routeCount`/`routes`/`vars` fields were stale
  (`routeCount: 0`, `routes: []`, `vars: []`); this port corrects them from
  the component's actual `wrangler.jsonc` (`routes[].pattern` and the
  `vars` map keys, sorted), same as the landed reference this was copied
  from (`cloud-itonami/app-tia`'s `tia-mcp-component/cljs`).

  Backend TypeScript is unaffected by this migration: this component ships
  no worker of its own (`../src/` contains only README.md, no `app.ts`).
  The one TypeScript file that lived under the deleted `svelte/` tree —
...
;; svelte/src/routes/+page.svelte to here. :routes, :route-count, and
;; :vars are corrected from wrangler.jsonc (the Svelte scaffold had stale
;; empty/zero values there).
...
(def default-db
  {:title "Kiyo K1y04rc4"
   ...
   :kind "wasm appview"
   :route-count 2
   :routes ["kiyo.etzhayyim.com/*" "k1y04rc4.etzhayyim.com/*"]
   :vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_DID" "APP_FRAMEWORK"
          "IPFS_GATEWAY_URL"]
   :xrpc? true
   :relative-path "wasm/etzhayyim-wasm-kiyo-k1y04rc4/cljs/src/kiyo/app.cljs"})
