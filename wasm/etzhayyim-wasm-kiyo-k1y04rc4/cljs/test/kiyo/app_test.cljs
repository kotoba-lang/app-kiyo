...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates the ported +page.svelte `app` object"
    ...
    (is (= "Kiyo K1y04rc4" @(rf/subscribe [:title])))
    (is (= "etzhayyim-project-kiyo" @(rf/subscribe [:project])))
    (is (= "etzhayyim-wasm-kiyo-k1y04rc4" @(rf/subscribe [:name])))
    (is (= "wasm appview" @(rf/subscribe [:kind])))
    (is (= 2 @(rf/subscribe [:route-count])))
    (is (= ["kiyo.etzhayyim.com/*" "k1y04rc4.etzhayyim.com/*"]
           @(rf/subscribe [:routes])))
    (is (= 4 (count @(rf/subscribe [:vars]))))
    (is (true? @(rf/subscribe [:xrpc?])))
    (is (= "wasm/etzhayyim-wasm-kiyo-k1y04rc4/cljs/src/kiyo/app.cljs"
...
(deftest vars-sub-carries-every-original-binding-name
  (testing "no runtime var name was dropped or renamed during the port"
    (rf/dispatch-sync [:initialize-db])
    (is (= #{"AGENTGATEWAY_MCP_ROUTER_URL" "APP_DID" "APP_FRAMEWORK"
             "IPFS_GATEWAY_URL"}
