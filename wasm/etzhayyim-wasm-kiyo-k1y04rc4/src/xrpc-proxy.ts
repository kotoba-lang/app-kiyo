...
// ../cljs/src/kiyo/app.cljs); backend TypeScript was to be left alone. It
// happened to live inside the now-deleted svelte/ tree, so it is moved here
// rather than deleted.
//
// It imports SvelteKit-specific modules (`@sveltejs/kit`, `./$types`) and
// therefore will not run as-is now that the SvelteKit build is gone:
...
//     static assets from `./cljs/public`). Unlike some sibling components,
//     this one ships no worker of its own — `../src/` held only README.md
//     before this file was moved in, so there is no existing dispatcher
//     Worker that could be pointed at as a replacement `main` either.
//
